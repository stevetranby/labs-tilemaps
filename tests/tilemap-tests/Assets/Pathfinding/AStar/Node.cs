...
namespace AStar
{
    ...
    public partial class Node
    {
		...
		// add nodes as neighbors (used when removing a wall/collision tile)
		public void AddNeighborsToNodes(List<Node> nodes) 
		{
			int cost = 1;
			foreach(Node node in nodes) {
				AddDirected(node, cost);
			}
		}

		// remove neighbors (effectively setting node as collision)
